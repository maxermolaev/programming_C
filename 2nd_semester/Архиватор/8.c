...
void printPrefTree(Node * T, int * lenCodingFile) { 
    if (checkLeaf(T)) { 
        printf("1"); 
        (* lenCodingFile)++; 
        for (int i = 0; i < 8; i++) { 
    ...
    printf("0"); 
    (* lenCodingFile)++; 
    printPrefTree(T->left, lenCodingFile); 
    printPrefTree(T->right, lenCodingFile); 
} 
...
int main() { 
    ...
    for (int i = 0; i < 8; i++) { 
        if ((unsigned char)heap->len & (unsigned char)1 << (7 - i)) { 
            printf("1"); 
        } 
        else { 
            printf("0"); 
    ...
    int lenCodingFile = 0; 
    printPrefTree(HuffmanTree, &lenCodingFile); 
    if (lenCodingFile % 8 != 0) { 
        for (int i = 0; i < 8 - (lenCodingFile % 8); i++) { 
            printf("0"); 
        } 
    } 
    ...
    for (int i = 0; i < fileSize; i++) { 
        lenCodingFile += encodings[inputFile[i]].len; 
        for (int j = 0; j < encodings[inputFile[i]].len; j++) { 
            if (encodings[inputFile[i]].code[j / 8] & ((unsigned char)1 << (7 - (j % 8)))) 
                printf("1"); 
            else 
                printf("0"); 
        }
